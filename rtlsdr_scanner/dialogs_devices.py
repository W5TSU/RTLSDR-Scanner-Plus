...
class DialogDevicesRTL(wx.Dialog):
    ...
    def __init__(self, parent, devices, settings):
        ...
        for i in range(self.COLS - 1):
            result = dc.GetMultiLineTextExtent(self.gridDev.GetColLabelValue(i))
            if len(result) == 2:
                _w, h = result  # newer wxPython returns (width, height)
            else:
                _w, h, _hl = result  # older wxPython returns (width, height, lineHeight)
            if h > maxHeight:
                maxHeight = h
        self.gridDev.SetColLabelSize(int(maxHeight * 1.25))
...
class DialogDevicesGPS(wx.Dialog):
    ...
    def __set_dev_grid(self):
        ...
        self.gridDev.SetColSize(self.COL_TYPE, int(width * 1.5))
