...
class FrameMain(wx.Frame):
    ...
    def __create_toolbars(self):
        ...
        grid2.AddGrowableCol(5)  # Changed from 3 to 5 to ensure Display column stays visible

        self.toolbar1.SetSizerAndFit(grid1)
        self.toolbar1.Layout()
        toolSize1 = self.toolbar1.GetMinSize()
        # Ensure minimum toolbar size to prevent GTK errors
        if toolSize1[0] <= 0 or toolSize1[1] <= 0:
            toolSize1 = (200, 40)
        ...
        # Ensure minimum toolbar size to prevent GTK errors
        if toolSize2[0] <= 0 or toolSize2[1] <= 0:
            toolSize2 = (200, 40)
        ...
        safeToolSize1 = (max(toolSize1[0], 100), max(toolSize1[1], 30))
        ...
        self._mgr.AddPane(self.toolbar1, aui.AuiPaneInfo().
                          ...
                          CloseButton(False).
                          MinimizeButton(True).
                          MinSize(safeToolSize1))
        # Ensure minimum size is reasonable
        safeToolSize2 = (max(toolSize2[0], 100), max(toolSize2[1], 30))
    ...
    def __set_size(self):
        ...
        # Ensure toolbar sizes are valid to prevent negative space calculations
        if toolSize1[0] <= 0 or toolSize1[1] <= 0:
            toolSize1 = (100, 30)  # fallback minimum size
        if toolSize2[0] <= 0 or toolSize2[1] <= 0:
            toolSize2 = (100, 30)  # fallback minimum size

        paneTool1 = self._mgr.GetPane(self.toolbar1)
        paneTool2 = self._mgr.GetPane(self.toolbar2)
        
        # Calculate required space for side-by-side layout
        minSpaceNeeded = toolSize1[0] + toolSize2[0] + (widthBorder * 2) + (widthFrame * 2) + 50
        
        if width >= minSpaceNeeded:
            paneTool1.ToolbarPane()
            paneTool2.ToolbarPane()
            paneTool2.Layer(1)
        self._mgr.Update()

        # Calculate minimum width based on the larger toolbar plus some margin
        minWidth = max(toolSize1[0], toolSize2[0]) + max(widthBorder, 0) + 50
        minWidth = max(minWidth, 750)  # Further reduced to prevent dropdown cutoff
        minHeight = 600

        # Ensure widthFrame is positive
        widthFrame = max(widthFrame, 0)
        ...
        # Calculate final window size with safety checks
        finalWidth = max(minWidth + (widthFrame * 2), int(width / 1.5), 800)
        finalHeight = max(minHeight + (widthFrame * 2), int(height / 1.5), 600)
