...
class PanelGraph(wx.Panel):
    ...
    def __init__(self, panel, notify, settings, status, remoteControl):
        ...
        self.figure.set_size_inches(0.1, 0.1)
        ...
        # Set minimum size to prevent GTK negative allocation errors
        toolbarSize = self.toolbar.GetSize()
        if toolbarSize[0] <= 0 or toolbarSize[1] <= 0:
            self.toolbar.SetMinSize((200, 35))
    ...
    def __on_size(self, event):
        ...
        # Validate canvas size to prevent negative/zero dimensions
...
class PanelMeasure(wx.Panel):
    ...
    def __init__(self, graph, settings):
        ...
        self.grid.SetColMinimalAcceptableWidth(5)  # Increased from 1 to prevent GTK errors
        self.grid.SetColSize(2, 5)   # Increased from 1 to prevent GTK allocation errors
        self.grid.SetColSize(7, 5)   # Increased from 1 to prevent GTK allocation errors
        self.grid.SetColSize(11, 5)  # Increased from 1 to prevent GTK allocation errors
        self.grid.SetColSize(15, 5)  # Increased from 1 to prevent GTK allocation errors
        ...
        dc.SetFont(font)
        widthMHz = int(dc.GetTextExtent('###.######')[0] * 1.2)
        widthdB = int(dc.GetTextExtent('-##.##')[0] * 1.2)
