        ...
        if self.extent is not None and self.plot is not None:
            ...
            if self.settings.autoL or force:
                ...
                try:
                    ...
                    self.barBase.mappable.set_clim(self.extent.get_l())
                try:
...
class ThreadPlot(threading.Thread):
    ...
    def __plot(self, spectrum):
        ...
        if self.settings.autoL:
            try:
                ...
                vmin, vmax = self.barBase.mappable.get_clim()
        else:
        ...
        if self.parent.settings.wireframe:
            try:
                # Try new matplotlib API first (rcount/ccount)
                self.parent.plot = \
                ...
                # Fallback to old API (rstride/cstride)
                self.parent.plot = \
                    self.axes.plot_wireframe(x, y, z,
                                             rstride=1, cstride=1,
                                             linewidth=0.1,
        ...
        # Set gid if supported (some matplotlib versions don't support gid for 3D plots)
        try:
            self.parent.plot.set_gid('plot_line')
    ...
    def __plot_peaks(self):
        ...
        # Convert to lists once for efficiency and Python 3 compatibility
        sweep_freqs = list(sweep.keys())
        sweep_values = list(sweep.values())
        
        for i in indices:
            self.axes.plot([sweep_freqs[i]], [lastTime], [sweep_values[i]],
                           linestyle='None',
