...
try:
    ...
    # matplotlib >= 3.3 removed seconds, use equivalent
    def seconds(x):
        return x * (1.0 / (24.0 * 3600.0))
import numpy
    ...
    def __calc_extent(self, spectrum):
        ...
        for timeStamp in spectrum:
            ...
            if len(points) > 0:
                ...
                if(lMax >= self.lMax):
                    self.lMax = lMax
                    self.fPeak, self.lPeak = max(points, key=lambda x: x[1])
...
def get_peaks(spectrum, threshold):
    ...
    # Create a new dictionary instead of modifying while iterating
    filtered_sweep = OrderedDict()
    for freq, level in sweep.items():
        if level >= threshold:
            filtered_sweep[freq] = level
    
    indices = (numpy.diff(numpy.sign(numpy.diff(list(filtered_sweep.values())))) < 0).nonzero()[0] + 1

    return filtered_sweep, indices
