...
try:
    ...
    # wx.animate might not be available in some wxPython versions
    AnimationCtrl = None
    Animation = None
...
class Statusbar(wx.StatusBar):
    ...
    def __init__(self, parent, log):
        ...
        if Animation is not None and AnimationCtrl is not None:
            ...
            # Fallback to a simple static control if animation not available
            busy = wx.StaticText(self, -1, "Busy")
            busy.SetToolTip('Updating plot')
            self.controls[4] = busy
    ...
    def set_busy(self, busy):
        ...
        animCtrl.Show(busy)
        if busy:
            if hasattr(animCtrl, 'Play'):
                animCtrl.Play()
        else:
            if hasattr(animCtrl, 'Stop'):
                animCtrl.Stop()
...
class NavigationToolbar(NavigationToolbar2WxAgg):
    def __init__(self, canvas, panel, settings, callBackHideOverlay):
        ...
        # Set standard toolbar tool size to prevent GTK allocation errors
        self.SetToolBitmapSize((24, 24))
        
        # Ensure toolbar has minimum size to prevent GTK allocation errors
        self.SetMinSize((200, 35))
        
        # Force initial layout to prevent negative allocation errors
        wx.CallAfter(self.__ensure_size)
        wx.CallAfter(self.__fix_tool_sizes)
    ...
    def __ensure_size(self):
        """Ensure toolbar has proper size to prevent GTK allocation errors"""
        size = self.GetSize()
        if size[0] < 200 or size[1] < 35:
            self.SetSize((max(size[0], 200), max(size[1], 35)))
            self.GetParent().Layout()
    
    def __fix_tool_sizes(self):
        """Fix individual tool sizes to prevent GTK allocation errors"""
        try:
            # Force realize and layout
            self.Realize()
            # Get a reasonable minimum size for tools
            min_tool_size = (24, 24)  # Standard icon size
            
            # Try to get tool count and ensure space
            tool_count = len([tool for tool in self.GetChildren() if hasattr(tool, 'GetBitmap')])
            if tool_count > 0:
                min_width = tool_count * (min_tool_size[0] + 4) + 50  # padding
                current_size = self.GetSize()
                if current_size[0] < min_width:
                    self.SetSize((min_width, max(current_size[1], min_tool_size[1] + 6)))
                    self.GetParent().Layout()
        except Exception:
            pass  # Ignore any errors during size fixing
