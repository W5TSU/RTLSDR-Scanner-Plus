...
import wx
try:
    from wx.grid import GridCellRenderer as PyGridCellRenderer
except ImportError:
...
class MultiButton(wx.Control):
    ...
    def DoGetBestSize(self):
        """Override to provide proper size calculation and prevent GTK allocation errors"""
        dc = wx.ClientDC(self)
        font = self.GetFont()
        dc.SetFont(font)
        ...
        for option in self.options:
            text_size = dc.GetTextExtent(option)
            max_width = max(max_width, text_size[0])
        
        # Add padding for button appearance and dropdown arrow
        width = max_width + (self.PADDING * 6) + self.ARROW_SIZE
        height = dc.GetTextExtent("Ay")[1] + (self.PADDING * 2)
        
        return wx.Size(max(width, 80), max(height, 25))
    ...
    def __draw(self, dc):
        ...
        dc.DrawLine(int(left), int(top), int(left), int(bottom))
